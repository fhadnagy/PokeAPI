...
class GetAllPokemonsByTypeUseCase @Inject constructor(
    private val repository: PokeRepository
){
    operator fun invoke(type : String) : Flow<Resource<List<RowPokemonData>>> = flow {
        try {
            emit(Resource.Loading())
            val pokemons = repository.getPokemonsByType(type)
